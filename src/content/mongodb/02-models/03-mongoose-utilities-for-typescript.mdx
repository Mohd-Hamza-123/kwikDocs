...
> 📌 Make sure you understand `Schema`, `model`, and `models` from the previous document.
...
✔ Safer refactoring (change schema ➝ TS updates automatically)
...
📌 **No manual typing needed — TypeScript infers it.**
...
> 📌 Use it when you work with *fetched* or *created* documents.
...
```ts
user.save();   // ❌ TS error
user._id;      // ❌ unknown type
```

📌 **Use HydratedDocument for CRUD operations.**
...
For example, **ObjectId ≠ string**, and **Decimal128 ≠ number**.
`Types` helps TypeScript understand the difference.

---

### 🔣 Major Types Provided by Mongoose
...
📌 `Types.Array<T>` enforces the type inside the array.
